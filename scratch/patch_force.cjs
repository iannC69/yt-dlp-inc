...
// The marker we want to find is:
const startMarker = '{showSettingsModal && (\\n          <motion.div \\n            className="settings-modal-overlay"';
// Actually, let's use regex matching carefully.
const regex = /{showSettingsModal && \([\s\S]*?className="settings-modal-overlay"[\s\S]*?className="settings-modal-content"[\s\S]*?<\/motion\.div>\s+<\/motion\.div>\s+\)}\s+<\/AnimatePresence>/m;

const replacement = `{showSettingsModal && (
          <motion.div 
            className="settings-modal-overlay"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
...
if (regex.test(content)) {
  ...
  console.log("Failed to match settings modal block");
}
