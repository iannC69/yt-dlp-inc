...
const regex = /<motion\.div\s+className="settings-modal-content"[\s\S]*?(?=<\/motion\.div>\s+<\/motion\.div>\s+<\/AnimatePresence>)/m;
...
if (regex.test(c)) {
  c = c.replace(regex, newModalCode);
  fs.writeFileSync('src/App.jsx', c, 'utf8');
  console.log('App.jsx modified with regex');
} else {
