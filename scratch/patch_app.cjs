...
// Add activeSettingsTab state
if (!c.includes("const [activeSettingsTab, setActiveSettingsTab]")) {
  c = c.replace(
...
// Extract everything from <motion.div className="settings-modal-content" to </motion.div> which ends the modal.
// We can just use a regex or string replacement if we are careful.

const oldContentStart = `            <motion.div 
...
const startIndex = c.indexOf(oldContentStart);
if (startIndex !== -1) {
  // Find the end of this div. It ends at `</motion.div>\n        )}`
  const endPattern = "</motion.div>\n        )}\n      </AnimatePresence>";
  const endIndex = c.indexOf(endPattern, startIndex);
  if (endIndex !== -1) {
